...
Процедура СоздатьРеквизиты(Форма, структураРеквизитов) Экспорт
	...
	СуществующиеРеквизиты = ТипыРеквизитовФормы(Форма);
	ДобавляемыеРеквизиты = Новый Массив;
	Для Каждого ТекущийРеквизит Из структураРеквизитов Цикл
		СуществующийТип = СуществующиеРеквизиты[ТекущийРеквизит.Ключ];
		Если СуществующийТип <> Неопределено Тогда
			ПроверитьСовпадениеТипаРеквизита(ТекущийРеквизит.Ключ, СуществующийТип, ТекущийРеквизит.Значение);
			Продолжить;
		КонецЕсли;
...
Процедура СоздатьРеквизитыТаблицы(Форма, ИмяТаблицы, структураРеквизитов) Экспорт
	...
	СуществующиеРеквизиты = ТипыРеквизитовФормы(Форма, ИмяТаблицы);
	ДобавляемыеРеквизиты = Новый Массив;
	Для Каждого ТекущийРеквизит Из структураРеквизитов Цикл
		СуществующийТип = СуществующиеРеквизиты[ТекущийРеквизит.Ключ];
		Если СуществующийТип <> Неопределено Тогда
...
// Возвращает имя вида по его номеру в многострочном наборе видов.
// При выходе номера за границы набора вызывает понятное исключение
// вместо неявной ошибки обращения к перечислению по пустому ключу.
//
// Параметры:
//  НаборВидов - Строка - многострочный список имён видов (по строке на вид)
//  НомерВида - Число - номер строки (вида), начиная с 1
//  ИмяПараметра - Строка - имя исходного параметра для текста исключения
//
// Возвращаемое значение:
//  Строка - имя вида (для обращения к соответствующему системному перечислению)
//
Функция ВидПоНомеру(НаборВидов, НомерВида, ИмяПараметра)
	ВидПоля = СокрЛП(СтрПолучитьСтроку(НаборВидов, НомерВида));
	Если ВидПоля = "" Тогда
		ВызватьИсключение "Недопустимое значение параметра """ + ИмяПараметра
			+ """: " + Строка(НомерВида) + ". Ожидается число от 1 до "
			+ Строка(СтрЧислоСтрок(НаборВидов)) + ".";
	КонецЕсли;
	Возврат ВидПоля;
КонецФункции
...
Функция СздПоле(Форма,
	...
	Если ШифрВидаПоля <> 0 Тогда
		...
		|ПолеПериода
		|ПолеHTMLДокумента";
		ВидПоля = ВидПоНомеру(НаборВидов, ШифрВидаПоля, "ШифрВидаПоля");
...
Функция СздПолеФлажок(Форма,
	...
	Если тВидФлажка <> 0 Тогда
		...
		|Тумблер	
		|Выключатель";
		ВидПоля = ВидПоНомеру(НаборВидов, тВидФлажка, "тВидФлажка");
...
Функция СздКнопка(Форма,
	...
	Если ВидКнопки <> 0 Тогда
		...
		|Гиперссылка";
		ВидПоля = ВидПоНомеру(НаборВидов, ВидКнопки, "ВидКнопки");
...
Функция СздКолонкаТаблицыФормы(Форма,
	...
	Если ТипКолонки <> 0 Тогда
		...
		|ПолеФлажка
		|ПолеКартинки";
		ВидПоля = ВидПоНомеру(НаборВидов, ТипКолонки, "ТипКолонки");
	Иначе
		ВидПоля = "ПолеВвода";
	КонецЕсли;	
...
Функция СздПереключатель(Форма,
	...
	Если тВидПереключателя <> 0 Тогда
		...
		|Тумблер";		
		ВидПоля = ВидПоНомеру(НаборВидов, тВидПереключателя, "тВидПереключателя");
...
// Возвращает соответствие имён реквизитов формы их типам значения.
// Список реквизитов запрашивается у платформы один раз, что избавляет
// от повторных обращений при массовой проверке существования реквизитов.
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - проверяемая форма
//  ИмяТаблицы - Строка - имя таблицы; пусто - корневые реквизиты формы
//
// Возвращаемое значение:
//  Соответствие - ключ - имя реквизита, значение - ОписаниеТипов реквизита
//
Функция ТипыРеквизитовФормы(Форма, ИмяТаблицы = "")
	Если ЗначениеЗаполнено(ИмяТаблицы) Тогда
		СписокРеквизитов = Форма.ПолучитьРеквизиты(ИмяТаблицы);
	Иначе
		СписокРеквизитов = Форма.ПолучитьРеквизиты();
	КонецЕсли;
	Результат = Новый Соответствие;
	Для Каждого ТекущийРеквизит Из СписокРеквизитов Цикл
		Результат.Вставить(ТекущийРеквизит.Имя, ТекущийРеквизит.ТипЗначения);
	КонецЦикла;
	Возврат Результат;
КонецФункции

// Сравнивает два описания типов по составу типов (без учёта квалификаторов).
//
// Параметры:
//  ОписаниеТипов1 - ОписаниеТипов - первое описание
//  ОписаниеТипов2 - ОписаниеТипов - второе описание
//
// Возвращаемое значение:
//  Булево - Истина, если наборы типов совпадают
//
Функция ТипыРеквизитаСовпадают(ОписаниеТипов1, ОписаниеТипов2)
	Типы1 = ОписаниеТипов1.Типы();
	Типы2 = ОписаниеТипов2.Типы();
	Если Типы1.Количество() <> Типы2.Количество() Тогда
		Возврат Ложь;
	КонецЕсли;
	Для Каждого ТекущийТип Из Типы1 Цикл
		Если Типы2.Найти(ТекущийТип) = Неопределено Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
	Возврат Истина;
...
// Проверяет, что существующий реквизит формы имеет требуемый тип; иначе вызывает исключение.
// Защищает от молчаливого переиспользования реквизита с другим типом значения
// при идемпотентном создании (например, строковый выпадающий список и числовой
// переключатель с одинаковым именем на одной форме).
//
// Параметры:
//  ИмяРеквизита - Строка - имя реквизита
//  СуществующийТип - ОписаниеТипов - тип уже существующего реквизита
//  ТребуемыйТип - ОписаниеТипов - тип, запрошенный при создании
//
Процедура ПроверитьСовпадениеТипаРеквизита(ИмяРеквизита, СуществующийТип, ТребуемыйТип)
	Если НЕ ТипыРеквизитаСовпадают(СуществующийТип, ТребуемыйТип) Тогда
		ВызватьИсключение "Реквизит формы """ + ИмяРеквизита
			+ """ уже существует с другим типом значения.";
	КонецЕсли;
КонецПроцедуры
